//! One check gates a pull request, and it stands for every job (#154).
//!
//! Branch protection names checks, and a check is a job name. This
//! repository used to name five of them -- `fmt`, `coverage` and the three
//! `test / <os>` matrix legs -- which meant the list had to be edited
//! whenever a job was renamed, a runner added to the matrix, or a job split
//! in two. Until someone did, the new work was required by nobody. The
//! opposite spelling is worse: a required check no job produces reads as
//! permanently pending, and with `enforce_admins` on nothing merges and
//! there is no failure to point at.
//!
//! Every driver in this family depends on this crate, so a job that quietly
//! stopped gating here is a regression that reaches all of them.
//!
