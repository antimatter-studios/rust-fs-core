...
//!
//! `include/fs_core.h` said "Link with libam_fs_core.a" for as long as the file
//! existed, and cargo has never produced that name: `[lib] name` is
//! `fs_core`, so the artefact is `libfs_core.a` — which is exactly what
...
//! lines, and rejected three spellings cargo accepts: `name = 'fs_core'`
//! in single quotes, `name = "fs_core" # comment` with a trailing
...
/// `Fs_core` produced `libFs_core.a`, the header's correct `Link with
/// libFs_core.a` was lowered to `libfs_core.a`, and the guard REJECTED a
...
/// saying `libfs_core.a` while cargo builds `libFs_core.a` would pass — and
...
/// libfs_core.a` satisfied a check for "does the header say to link it".
///
...
fn the_link_instruction_must_open_the_sentence() {
    let want = "libfs_core.a";

    for accepted in [
        " * Link with libfs_core.a alongside this header.\n",
        "Link with libfs_core.a\n",
        "// Link with libfs_core.a and include this header.\n",
        "  # link with   libfs_core.a\n",
        " */ Link with libfs_core.a\n",
    ...
    for refused in [
        // The filed defect.
        " * Do not link with libfs_core.a; it is an implementation detail.\n",
        " * You must never link with libfs_core.a directly.\n",
        // A mention with no instruction: what #76's first fix caught.
        " * The build produces libfs_core.a in the target directory.\n",
        " * libfs_core.a was renamed in 0.4.0.\n",
...
/// declaring `[lib] name = "Fs_core"` had its correct `Link with
/// libFs_core.a` rejected — in the packaging step, after a green CI.
...
fn a_mixed_case_library_name_is_read_as_itself() {
    ...
    for spelling in [
        ...
        assert!(
            states_the_link_instruction(spelling, "libfs_core.a"),
            "{spelling:?} is the instruction; only its case differs"
        );
    }

    assert!(
        !states_the_link_instruction(" * Link with libfs_core.a\n", "libFs_core.a"),
        "cargo builds libFs_core.a and the header says libfs_core.a. A linker is \
...
fn the_header_scan_finds_library_names_in_prose() {
    let header = " * Link with libfs_core.a (or its sister-crate equivalent that\n";
    assert_eq!(libraries_named(header), vec!["libfs_core.a"]);

    // The exact defect, and the near-miss that has to stay distinct.
    assert_eq!(
        libraries_named(" * Link with libam_fs_core.a and include this\n"),
        vec!["libam_fs_core.a"]
    );
...
fn the_libname_is_parsed_rather_than_scanned() {
    for (what, yml) in [
        ...
        (
            "a trailing comment",
            "vars:\n  LIBNAME: fs_core # the linked name\n",
        ),
        (
