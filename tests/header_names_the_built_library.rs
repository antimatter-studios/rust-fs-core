...
//!
//! `include/fs_core.h` said "Link with libam_fs_core.a" for as long as
//! the file existed, and cargo has never produced that name: `[lib]
//! name` is `fs_core`, so the artefact is `libfs_core.a`. A C consumer
//! following the first paragraph of the first file they read got a
...
//! not a naming rule anybody can apply once — it is a thing that drifts
//! and therefore wants a check.
//!
...
/// `[lib] name` out of `Cargo.toml`.
///
/// A hand parse rather than a toml dependency: this crate deliberately
/// has none, and a test is not a reason to acquire one.
...
fn the_header_tells_consumers_to_link_the_library_that_is_built() {
    ...
    // otherwise make the loop below pass over an empty list, which is
    // the shape of defect this file exists for.
    assert!(
        !named.is_empty(),
...
fn the_header_scan_finds_library_names_in_prose() {
    let header = " * Link with libfs_core.a (or its sister-crate equivalent)\n";
